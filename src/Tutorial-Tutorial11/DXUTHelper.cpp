...
void DXUTHelper::init_gui()
{
	...
	WCHAR sz[100];
	iY += 24;
	swprintf_s(sz, 100, L"Puffiness: %0.2f", m_fModelWaviness);
	m_SampleUI.AddStatic(IDC_PUFF_STATIC, sz, 0, iY += 26, 170, 22);
	m_SampleUI.AddSlider(IDC_PUFF_SCALE, 50, iY += 26, 100, 22, 0, 2000, ( int )( m_fModelWaviness * 100.0f ));
...
void CALLBACK DXUTHelper::AppGUIEvent(UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext)
{
	...
	{
		WCHAR sz[100];
		m_fModelWaviness = ( float )( m_SampleUI.GetSlider(IDC_PUFF_SCALE)->GetValue() * 0.01f );
		swprintf_s(sz, 100, L"Puffiness: %0.2f", m_fModelWaviness);
